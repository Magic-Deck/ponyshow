...
function renderFile(dp,_cb) {
  ...
  fs.readFile(dp.fullPath, 'utf8', function (err, str) {
    ...
    var dat = JSON.parse(fs.readFileSync(path.join(__dirname,'../config.json')).toString());
...
Module.prototype.init = function (_opts) {
  ...
  // Install ponypath and workspace
  mkdirp.sync(ponypath);
  mkdirp.sync(ponypath+'/workspace');

  // synchronize workspace assets
  ncp(path.join(path.join(__dirname,'../workspace')), path.join(ponypath,'/workspace'), function (err) {
   if (err) {
     return console.error(err);
   }
   parser.parse();
