/**
 * Main entry point for the Ponyshow. Responsible for loading the command line
 * configuration, initializing the i18n system, defining global options and
 * flags, and running the main logic.
 *
 * @module ponyshow
 *
 * @copyright
...
function promptForNewDeck(args) {
                    ...
                    request.get(opts)
                      ...
                      .on('end',function() {
                        ...
                        read1.on("finish", function() {
                          ...
                          console.log('Deck generated: '+target.yellow);
                          console.log('Run Ponyshow to view the deck.');
                          process.exit(0);
                        });
...
function Module(_opts) {
  ...
  function init(_opts) {
    ...
    parser.command('new')
      .callback(this.new).help("new deck or theme");
    parser.command('run')
      .callback(this.run).help("run a presentation file(s)");
    parser.command('install')
      .callback(this.install).help("install themes and projects");
    parser.command('themes')
      .callback(this.themes).help("view installed themes");
  ...
  function config(_opts) {
    ...
    // CLI
    if (args.hasOwnProperty('_') && self.callee == "cli") {
  ...
  function themes(_opts, _cb) {

    spinnerctx = new Spinner('%s Looking for themes..');
    spinnerctx.setSpinnerString('|/-\\');
    spinnerctx.start();

    // detect if current dir is ponyfile
    readdirp({ root: workspace + '/themes', fileFilter: [ 'package.json' ] },function(entry) {

      try {
          // stash commands for future deck functions
          entry.args = _opts||{};
          entry.theme = JSON.parse(fs.readFileSync(entry.fullParentDir+'/package.json'))
      } catch (err) {
          console.error(err);
      }
      ...
      if (!res) {
       ...
       if (res.files.length > 0) {
         ...
         console.log('Installed themes:'.yellow+'\n');

         for (var x in res.files) {
           console.log('\t'+res.files[x].theme.name.replace('ponyshow-theme-',''));
         }
  ...
  function run(_opts) {
      ...
      spinnerctx = new Spinner('%s Looking for decks..');
      spinnerctx.setSpinnerString('|/-\\');
      spinnerctx.start();
